...
public class MainActivity extends AppCompatActivity {
    ...
    boolean ischanged=false; //기존 프로필 이미지를 변경한적이 있는가?
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //ShardPreference 에 미리 저장되어 있는 닉네임과 프로필이 이미지가 있다면 읽어오기
        loadData();
        if (G.nickname!=null){
    ...
    //ShardPreference 의 저장값을 읽어오는 기능 메소드
    void loadData(){
    ...
    public void clickBtn(View view) {
        ...
        if(isFirst || ischanged) {
            //닉네임과 프로필이미지로 Firebase storage 와 DB에 저장 후 채팅화면으로
            saveData();
        }else {
            //저장없이 곧바로 채팅화면으로 전환
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                imgRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        //다운로드 주소 uri 을 문자열로 얻어오기
                        G.profileUrl=uri.toString();
                        Toast.makeText(MainActivity.this, "프로필이미지 저장완료+\n"+G.profileUrl, Toast.LENGTH_SHORT).show();

                        //1. 서버의 DB에 닉네임과 이미지 url 을 저장
                        //Firebase Database 에저장
                        ...
                        //2. ShardPreference 에 저장 (내부저장소에 데이터를 영구히 저장하는 녀석)
                        SharedPreferences pref= getSharedPreferences("account",MODE_PRIVATE);
